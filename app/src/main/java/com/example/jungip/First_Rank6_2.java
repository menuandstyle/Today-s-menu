...
public class First_Rank6_2 extends Activity implements OnClickListener {
Button mBtnAnim;
Button bt_first_rank_main7_2_decisionend;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.first__rank6_2);
		mBtnAnim = (Button)findViewById(R.id.bt_first_rank_main7_2_pen);
		
		bt_first_rank_main7_2_decisionend = (Button)findViewById(R.id.bt_first_rank_main7_2_decisionend);
		bt_first_rank_main7_2_decisionend.setOnClickListener(this);
	...
	public void mOnClick(View v) {
		...
		if (i==0) {
			switch (v.getId()) {
			case R.id.bt_first_rank_main7_2_pen:
				// 평면으로 돌고, 가로 세로 3배씩 커진다.
				anim = AnimationUtils.loadAnimation(this, R.anim.pen_90);
				mBtnAnim.startAnimation(anim); // 평면으로 0.1초간 6번 돌고, 가로 세로 4배씩 0.01초간 60번 커진다.
				break;
			}
		} else if (i==1) {
			switch (v.getId()) {
	...
	public void onClick(View v) {
		// TODO 자동 생성된 메소드 스텁
		switch(v.getId()){
		case R.id.bt_first_rank_main7_2_decisionend:
			moveTaskToBack(true);
			break;
		}
	}
